...
export const Route = createFileRoute("/contact")({
  head: () => {
    ...
    const description =
      "Have questions about funding? Reach out to Drawbridge Lending. Fast responses, no obligation, 100% confidential.";
    return buildHead({
      title,
      description,
      path: "/contact",
...
function ContactPage() {
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {/* Hero */}
      <section className="relative overflow-hidden bg-[color:var(--brand-charcoal)] py-12 text-white">
        <div aria-hidden className="pointer-events-none absolute inset-0 anchor-grid opacity-[0.07]" />
        <div aria-hidden className="pointer-events-none absolute -top-32 right-0 h-[400px] w-[400px] rounded-full blur-[130px]" style={{ background: "radial-gradient(circle, oklch(0.75 0.13 85 / 0.10) 0%, transparent 65%)" }} />

        <div className="relative mx-auto max-w-2xl px-6 text-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-[color:var(--brand-bronze)]/30 bg-[color:var(--brand-bronze)]/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-[color:var(--brand-bronze)]">
            Talk to Drawbridge Lending
          </div>
          ...
          <p className="mx-auto mt-3 text-sm text-white/55">
            Questions or ready to apply? Our funding desk is typically back within the hour on business days.
          </p>
          <div className="mt-5 flex flex-wrap items-center justify-center gap-4 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/40">
            <span className="inline-flex items-center gap-1.5"><ShieldCheck className="h-3 w-3 text-[color:var(--brand-bronze)]" /> 100% Confidential</span>
            <span className="inline-flex items-center gap-1.5"><CheckCircle2 className="h-3 w-3 text-[color:var(--brand-bronze)]" /> Soft Pull Only</span>
            <span className="inline-flex items-center gap-1.5"><Clock className="h-3 w-3 text-[color:var(--brand-bronze)]" /> 24-Hour Decisions</span>
          </div>
        </div>
      </section>

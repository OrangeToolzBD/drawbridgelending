...
export const Route = createFileRoute("/pillar/$slug")({
  head: ({ params }) => {
    ...
    const title = p ? `${p.title} in New Orleans, LA` : "Loan Program";
    const description = p?.description ?? "New Orleans business financing programs from Drawbridge Lending.";
      ...
      <main>
        ...
        <section className="relative overflow-hidden bg-[color:var(--brand-charcoal)] py-24 text-white">
          ...
          <div className="relative mx-auto max-w-4xl px-6">
            ...
            <h1 className="mt-5 text-4xl font-bold leading-tight tracking-tight md:text-6xl">
              {pillar.title}
              <span className="block text-[color:var(--brand-bronze)]">in New Orleans</span>
            </h1>
        ...
        <section className="bg-[color:var(--brand-cream)] py-16">
          <div className="mx-auto max-w-4xl px-6">
            ...
            <h2 className="mt-3 text-2xl font-bold tracking-tight md:text-3xl">
              {pillar.title} for New Orleans businesses
            </h2>
            {(PILLAR_BODIES[pillar.slug] ?? []).map((p, i) => (
              <p key={i} className="mt-4 text-muted-foreground">{p}</p>
            ))}
            <p className="mt-4 text-muted-foreground">
              Every Drawbridge Lending application runs through Louisiana-licensed lending partners --
        ...
        <section className="py-16">
          <div className="mx-auto max-w-7xl px-6">
            ...
            <h2 className="mt-3 text-2xl font-bold tracking-tight md:text-3xl">
              {pillar.title} by New Orleans neighborhood
            </h2>

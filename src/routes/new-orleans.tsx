...
const siteFavUrl = "/drawbridgefav.png";

export const Route = createFileRoute("/new-orleans")({
  head: ({ matches, match }) => {
    if (matches[matches.length - 1]?.routeId !== match.routeId) return {};
    const title = "New Orleans Business Loans Hub";
    const description =
      "The complete New Orleans, LA business funding directory: 24 loan programs across 10 neighborhoods and adjacent communities, from Downtown to Marigny, Garden District, Metairie and beyond.";
    return buildHead({
      title,
      description,
      path: "/new-orleans",
      schema: buildGraph({ title, description, path: "/new-orleans", pageType: "CollectionPage" }),
    });
  },
  component: NewOrleansHub,
});

function NewOrleansHub() {
  const matches = useMatches();
  const isLeaf = matches[matches.length - 1]?.routeId === "/new-orleans";
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main>
        ...
        <section className="relative overflow-hidden bg-[color:var(--brand-charcoal)] py-28 text-white">
          ...
          <div className="relative mx-auto max-w-5xl px-6 text-center">
            <div className="inline-flex items-center gap-2 rounded-full border border-[color:var(--brand-bronze)]/30 bg-[color:var(--brand-bronze)]/10 px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.18em] text-[color:var(--brand-bronze)]">
              <MapPin className="h-3.5 w-3.5" /> The New Orleans Directory
            </div>
            ...
            <p className="mx-auto mt-5 max-w-2xl text-lg text-white/65">
              The complete Drawbridge Lending directory for New Orleans, LA - every loan program we broker, mapped to every neighborhood and adjacent community we serve.
            </p>
        ...
        <PillarGrid
          ...
          subtitle="The 14 core funding programs Drawbridge Lending brokers for New Orleans businesses, with the six active GBP plays first."
          icon={Banknote}
        ...
        <PillarGrid
          ...
          subtitle="10 industry-specialized lending programs for Southeast Louisiana operators."
          icon={Briefcase}
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-1 max-w-2xl text-muted-foreground">
              Every neighborhood we serve, mapped to the six GBP-active money pillars New Orleans owners ask for most.
            </p>
          </div>
        </div>

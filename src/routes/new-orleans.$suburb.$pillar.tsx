...
export const Route = createFileRoute("/new-orleans/$suburb/$pillar")({
  head: ({ params }) => {
    const s = getSuburb(params.suburb);
    const p = getPillar(params.pillar);
    const title = s && p ? `${p.title} in ${s.name}, LA` : "New Orleans Loan Program";
    const description =
      s && p
        ? `${p.title} for ${s.name} businesses: ${p.tagline.toLowerCase()}. ${p.highlight}. Soft credit pull only.`
        : "New Orleans business funding programs.";
    const path = `/new-orleans/${params.suburb}/${params.pillar}`;
    return buildHead({
      title,
      description,
      path,
        ...
        <section className="relative overflow-hidden bg-[color:var(--brand-charcoal)] py-24 text-white">
          ...
          <div className="relative mx-auto max-w-4xl px-6">
            ...
            <p className="mt-4 max-w-2xl text-base text-white/65 md:text-lg">
              ...
              tied to New Orleans's I-10 corridor economy and the broader Southeast Louisiana market.
            </p>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/45">
              ...
              <span className="inline-flex items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 text-[color:var(--brand-bronze)]" /> LA-licensed partners</span>
            </div>
          </div>
        </section>

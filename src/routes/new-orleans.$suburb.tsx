...
export const Route = createFileRoute("/new-orleans/$suburb")({
  head: ({ params, matches, match }) => {
    if (matches[matches.length - 1]?.routeId !== match.routeId) return {};
    const s = getSuburb(params.suburb);
    const title = s ? `Business Loans in ${s.name}, LA` : "New Orleans Neighborhood";
    const description = s?.intro?.slice(0, 158) ?? "New Orleans neighborhood business funding.";
    const path = `/new-orleans/${params.suburb}`;
    return buildHead({
      title,
      description,
      path,
      schema: buildGraph({
        ...
        description,
        path,
        extraNodes: s ? [placeNode({ path, name: `${s.name}, LA` })] : [],
      }),
    });
  },
  loader: ({ params }) => {
...
function SuburbPage() {
  ...
  const isLeaf = matches[matches.length - 1]?.routeId === "/new-orleans/$suburb";
      ...
      <main>
        ...
        <section className="py-16">
          <div className="mx-auto max-w-7xl px-6">
            ...
            <h2 className="mt-3 text-2xl font-bold tracking-tight md:text-3xl">
              {suburb.name} businesses Drawbridge Lending works with
            </h2>
